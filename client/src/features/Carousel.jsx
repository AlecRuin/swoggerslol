        ...
        <a ref={ref} className="carousel-item w-25 h-a no-decor" target="_blank">
            <div className="video-holder">
                ...
                <video className="image-stretch w-100 carousel-video" autoPlay muted>
                    <source  className="source" type="video/mp4"/>
                </video>
            </div>
...
const CarouselItem = forwardRef((props,ref)=>{
    ...
    return(
        <a ref={ref} className={`carousel-item w-25 h-a no-decor ${classNames[props.id]}`} target="_blank" href={props.videos[selectedVid].youtube_link}>
            <video onEnded={changeVideo} className="image-stretch w-100 carousel-video" autoPlay muted>
...
export default function Carousel(){
    ...
    useEffect(()=>{
        fetch("/data/showcases.json").then(res=>{
            res.json().then(body=>{
                setVideos(body)
    ...
    return (
        <div className="basic-text-style w-100 carousel">
        {
            ...
            <CarouselItem
                ...
                ref={(el)=>{
                    itemsRef.current[index]=el
                    if(el)videosRef.current[index]=el.querySelector(".carousel-video")
                    if(index===2)setRefsReady(true)
                }}
            />)
            )
